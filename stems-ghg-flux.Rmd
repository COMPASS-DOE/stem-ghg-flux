...
```{r}
tree_dat %>%
  mutate(Season=if_else(yday(Date) >100 & yday(Date) <300, 
                        true = "Growing",
                        false = "Non-growing")) -> tree_dat
...
ggplot(data = tree_dat_ac, 
       mapping = aes(x = Date, y = CO2_lin_flux.estimate, color = Species)) +
  geom_point(alpha = 0.7, size = 2) +   # slightly larger, semi-transparent points
  labs(
    title = "CO2 Flux from Tree Stems Over Time by Species",
    x = "Date",
    y = expression("CO2 Flux (g CO2 / m² / day)"),  # uses proper units
    color = "Tree Species"
  ) +
  theme_minimal() 
...
```{r expanding this CO2 idea}
ggplot(data = tree_dat_ac, 
       mapping = aes(x = Date, y = CO2_lin_flux.estimate, color = Species)) +
  geom_point() +
  facet_grid(Species ~ Plot) +        # add this line
  labs(
    title = "CO2 Flux by Date and Plot",
    x = "Date",
...
tree_ch4_no_S7 <- tree_dat %>%
  filter(ID != "S7")   # remove tree S7

ggplot(tree_ch4_no_S7, aes(x = Date, y = CH4_lin_flux.estimate, color = Species)) +
  geom_point() +
  labs(
    title = "Practice",
    x = "Date",
    y = "CH4 Linear Flux Estimate"
  ) +
  theme_minimal()
...
```{r better version of CH4 all plots}

ggplot(data = tree_dat, 
       mapping = aes(x = Date, y = CH4_lin_flux.estimate, color = Species)) +
  geom_point(alpha = 0.7, size = 2) +   # semi-transparent, slightly larger points
  labs(
    title = "CH₄ Flux from Tree Stems Over Time by Species",
    x = "Date",
    y = expression("CH₄ Flux (g CH₄ / m² / day)"),  # proper units
    color = "Tree Species"
  ) +
  theme_minimal() 
...
```{r this but without s7}

ggplot(data = tree_dat %>% filter(ID != "S7"), 
       mapping = aes(x = Date, y = CH4_lin_flux.estimate, color = Species)) +
  geom_point(alpha = 0.7, size = 2) +   # semi-transparent, slightly larger points
  labs(
    title = "CH₄ Flux from Tree Stems Over Time by Species (Excluding Tree S7)",
    x = "Date",
    y = expression("CH₄ Flux (g CH₄ / m² / day)"),  # proper units
    color = "Tree Species"
  ) +
...
=======
# Compute for each year, plot, sapflux_ID mean annual flux
# Visualize using geom_boxplot where color is plot and facet is spp
>>>>>>> Stashed changes
```
...
avg_CH4flux_stats_filtered <- tree_dat %>%
  ...
  facet_grid(Species~ Plot) +   # <- split panels by plot
...
tree_soil_plot_CO2 <- tree_dat %>%
  mutate(
    Date = as.Date(Date),
    Plot = recode(Plot, "C"="Control","F"="Freshwater","S"="Salt")
  ) %>%
  left_join(
    soildat %>%
      mutate(
        Date = as.Date(Date),
        Plot = recode(Plot, "C"="Control","F"="Freshwater","S"="Saltwater
                    ")
      ) %>%
      group_by(Plot, Date) %>%
      summarise(
        soil.temp.5cm = mean(soil.temp.5cm, na.rm = TRUE),
        .groups = "drop"
      ),
    by = c("Plot", "Date")
  ) %>%
  filter(
    is.finite(soil.temp.5cm),
    is.finite(CO2_lin_flux.estimate)
  )
tree_soil_plot_CO2 <- tree_soil_plot_CO2 %>%
  mutate(Plot = recode(Plot, "Seawater" = "Saltwater"))  
...
ggplot(tree_soil_plot_CO2,
  ...
  labs(
    x = "Soil Temperature (5 cm, C)",
    y = "CO2 Flux (g CO2 / m2 / day)",
    title = "CO2 Flux vs Soil Temperature by Plot"
  )
```
# same for ch4
...
tree_soil_plot_CH4 <- tree_dat %>%
  ...
  filter(
    is.finite(soil.temp.5cm),
    is.finite(CH4_lin_flux.estimate)
  )
  
tree_soil_plot_CH4 <- tree_soil_plot_CH4 %>%
  mutate(Plot = recode(Plot, "Seawater" = "Saltwater"))  

ggplot(tree_soil_plot_CH4,
       aes(x = soil.temp.5cm,
...
#CH4 vs soil moisture?

```{r}
tree_soil_vwc <- tree_dat %>%
  mutate(Date = as.Date(Date)) %>%
  left_join(
    soildat %>%
      mutate(Date = as.Date(Date)) %>%
      group_by(Plot, Date) %>%
      summarise(
        soil.vwc.5cm = mean(soil.vwc.5cm, na.rm = TRUE),
        .groups = "drop"
      ),
    by = c("Plot", "Date")
  )

tree_soil_vwc_plot <- tree_soil_vwc %>%
  filter(ID != "S7") %>%
  filter(
    !is.na(soil.vwc.5cm),
    !is.na(CH4_lin_flux.estimate)
...
ggplot(tree_soil_vwc_plot,
  ...
  facet_wrap(~ Plot) +
  labs(
    title = "CH4 Flux vs Soil Moisture",
    x = "Soil Volumetric Water Content (VWC)",
    y = "CH4 Flux (g CH4 / m² / day)",
    color = "Species"
  ) +
  theme_minimal()
```
#Flag color between leaf on vs leaf off for soil temp data? May 1st - Sep 1st?
